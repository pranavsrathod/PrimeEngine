...
void DX9_KeyboardMouse::generateButtonEvents()
{
...
#endif
	{
		...
		if (m_leftButtonDown && (GetKeyState(VK_LBUTTON) & 0x8000) == 0) {
			POINT pt;
			Handle h("EVENT", sizeof(Event_LEFT_CLICK));
			Event_LEFT_CLICK* pEvt = new (h) Event_LEFT_CLICK;
			GetCursorPos(&pt);
			ScreenToClient(pWinApp->getWindowHandle(), &pt);
			pEvt->m_x = pt.x;
			pEvt->m_y = pt.y;
			m_pQueueManager->add(h, Events::QT_INPUT);
			// PEINFO("PE : Trigger : LEFT Click! \n");
			m_leftButtonDown = false;
		}
		if ((GetKeyState('T') & 0x8000) != 0) {
			m_tButton = true;
		}
		if (m_tButton && (GetKeyState('T') & 0x8000) == 0) {
