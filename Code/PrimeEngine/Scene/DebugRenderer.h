...
struct DebugRenderer : public SceneNode
{
	...
	void createRectangleMesh(Vector3 screenPos, float width, float height, float scale, float timeToLive, int& threadOwnershipMask);
	...
	void createScreenLineMesh(float* pRawData, int numPoints, float timeToLive = 0.2f);
	void debugRenderLines(Vector3 corners[]);
	void createScreenSpaceLineMesh(float* pRawData, int numPoints, float timeToLive);
	void createLineOverlay2D(float* lineData, int numPoints, float scale, float timeToLive, int& threadOwnershipMask);
	void DrawAABB(AABB& aabb, float timeToLive);
	void calculateAABB(MeshInstance* pInst, Mesh* pMeshCaller);
	...
	private:
		...
		Handle m_hButtonSNPool[NUM_ButtonSceneNodes];
		int m_hAvailableButtonSNs[NUM_ButtonSceneNodes];
		int m_hFreeingButtonSNs[NUM_ButtonSceneNodes];
		float m_buttonLifetimes[NUM_ButtonSceneNodes];
		int m_numAvailableButtonSNs;
		int m_numFreeingButtonSNs;
