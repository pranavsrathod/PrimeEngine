...
void DefaultAnimationSM::do_SCENE_GRAPH_UPDATE(Events::Event *pEvt)
{
	...
	for (PrimitiveTypes::UInt32 iSlot = 0; iSlot < m_animSlots.m_size; iSlot++)
	{
		...
		if (framesLeft < 0.0f)
		{
			PEINFO("Current Animation: %d", slot.m_animationIndex);
...
void DefaultAnimationSM::do_PRE_RENDER_needsRC(PE::Events::Event *pEvt)
{
	...
	Matrix4x4 base = pSN->m_worldTransform;

	// **Base Position for Debug Text (Vampire's Feet)**
	Vector3 textPos = base.getPos();

	// **Display Animation Type (Full Body or Partial)**
	char buf[80];
	sprintf(buf, "%s Animation", m_isFullBody ? "Full Body" : "Partial");
	pDebugRenderer->createTextMesh(buf, false, false, true, false, 0,
		textPos, 0.01f, pRealEvt->m_threadOwnershipMask);

	// Move text position **downward** slightly for next line
	textPos.m_y -= 0.15f;

	// **Find Active Animation Slot**
	AnimationSlot* pActiveSlot = nullptr;
	for (PrimitiveTypes::UInt32 i = 0; i < m_animSlots.m_size; i++)
	{
		if (m_animSlots[i].isActive())
		{
			pActiveSlot = &m_animSlots[i];
			break;
		}
	}

	// **Display Active Animation Details**
	if (pActiveSlot)
	{
		char animBuf[120];
		if (m_isFullBody) {
			sprintf(animBuf, "Anim: %d", pActiveSlot->m_animationIndex);
		}
		else {
			sprintf(animBuf, "Anim: 18 (Upper) , 3 (Lower)");
		}
		pDebugRenderer->createTextMesh(animBuf, false, false, true, false, 0,
			textPos, 0.01f, pRealEvt->m_threadOwnershipMask);

		textPos.m_y -= 0.15f;  // Move down for next line

		sprintf(animBuf, "Frame: %.2f / %.2f", pActiveSlot->m_frameIndex, pActiveSlot->m_numFrames);
		pDebugRenderer->createTextMesh(animBuf, false, false, true, false, 0,
			textPos, 0.01f, pRealEvt->m_threadOwnershipMask);

		textPos.m_y -= 0.15f;  // Move down for next line

		sprintf(animBuf, "Blend: %.2f", pActiveSlot->m_blendFactor);
		pDebugRenderer->createTextMesh(animBuf, false, false, true, false, 0,
			textPos, 0.01f, pRealEvt->m_threadOwnershipMask);
	}
	else
	{
		pDebugRenderer->createTextMesh("No Active Animation", false, false, true, false, 0,
			textPos, 0.01f, pRealEvt->m_threadOwnershipMask);
	}
	*/
	...
	PrimitiveTypes::Float32 customPercentage)
{
	...
	//m_isFullBody = !(additionalFlags & PE::PARTIAL_BODY_ANIMATION);
	//if (m_isFullBody) {
	//	AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset), 0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1), anim.m_startJoint, anim.m_endJoint, ACTIVE | additionalFlags /*| PARTIAL_BODY_ANIMATION | NOTIFY_ON_ANIMATION_END*/, weight);
	//	setSlot(goodSlot, slot);
	//	return &m_animSlots[goodSlot];
	//}
	//else {
	//	//AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset), 0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1), 4, 54, ACTIVE | additionalFlags /*| PARTIAL_BODY_ANIMATION | NOTIFY_ON_ANIMATION_END*/, weight);
	//	//
	...
	//		// Upper-body animation (Spine, Arms, Head)
	//		AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset),
	//			0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1),
	//			4, 54, // Upper body range
	//			ACTIVE | additionalFlags, weight);
	//		setSlot(goodSlot, slot);
	//	}
	//	else if (animationIndex == 3) {
	//		// Lower-body animation (Hips, Legs)
	//		AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset),
	//			0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1),
	//			55, 64, // Lower body range
	//			ACTIVE | additionalFlags, weight);
	//		setSlot(goodSlot, slot);
	//	}
	//}
	// AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset), 0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1), 0, 20, ACTIVE | additionalFlags /*| PARTIAL_BODY_ANIMATION | NOTIFY_ON_ANIMATION_END*/, weight);

	//setSlot(goodSlot, slot);
	//// Determine if the animation is full-body or partial-body
	//m_isFullBody = !(additionalFlags & PE::PARTIAL_BODY_ANIMATION);
	//return &m_animSlots[goodSlot];
}
