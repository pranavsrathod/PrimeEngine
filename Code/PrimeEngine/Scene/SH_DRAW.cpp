...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	if (pDrawEvent != NULL && false) {
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			DebugRenderer::Instance()->calculateAABB(pInst, pMeshCaller);
		}

	}
	if (pMeshCaller->m_performBoundingVolumeCulling && false)
	{
		pMeshCaller->m_numVisibleInstances = 0;

		// Fetch the active camera and its frustum planes
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			AABB aabb = pInst->m_bbTransformed;

			bool inside = true;
			for (int i = 0; i < 6; ++i) // Iterate over all frustum planes
			{
				Vector3 normal = pCamera->frustum.planes[i].normal;
				float distance = pCamera->frustum.planes[i].distance;

				int outcount = 0;
				for (int j = 0; j < 8; ++j) // Check all 8 corners of AABB
				{
					if (normal.dotProduct(aabb.corners[j]) + distance < 0.0f)
					{
						outcount++;
					}
				}
				if (outcount == 8) // If all corners are outside a plane Cull
				{
					inside = false;
					break;
				}
