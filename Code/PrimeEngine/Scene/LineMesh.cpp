...
void LineMesh::loadFrom3DPoints_needsRC(float *vals, int numPoints, const char *techName, int &threadOwnershipMask)
{
	...
	if (!m_loaded)
	{
		...
		Handle hEffect = EffectManager::Instance()->getEffectHandle(techName && techName[0] ? techName : "ColoredMinimalMesh_Line_A_0_Tech");

		// Handle hEffect = EffectManager::Instance()->getEffectHandle("StdMesh_2D_Diffuse_A_RGBIntensity_Tech");
