...
void GameObjectManager::do_CREATE_MESH(Events::Event *pEvt)
{
	...
	if (!haveObject)
	{
		...
		m_pContext->getGPUScreen()->AcquireRenderContextOwnership(pRealEvent->m_threadOwnershipMask);
			...
			// Create the SceneNode for the static object (building, car, etc.)
			PE::Handle hStaticSN("SCENE_NODE", sizeof(SceneNode));
			SceneNode* pStaticSN = new(hStaticSN) SceneNode(*m_pContext, m_arena, hStaticSN);
			pStaticSN->addDefaultComponents();
			pStaticSN->m_base.setPos(pRealEvent->m_pos); // Set its position

			/*
			// Create a PhysicsComponent (Box Collider) and attach it to SceneNode
			PhysicsComponent* pc = new PhysicsComponent(PhysicsComponent::BOX, pStaticSN);
			pc->setCenter(pRealEvent->m_pos);


			pc->box.min = pRealEvent->m_pos - Vector3(1.0f, 1.0f, 1.0f);
			pc->box.max = pRealEvent->m_pos + Vector3(1.0f, 1.0f, 1.0f);

			PhysicsManager::getInstance().addComponent(pc); // Register with PhysicsManager

			pStaticSN->pc = pc; // Attach PhysicsComponent to SceneNode
			*/

			// Register SceneNode in the game world
			RootSceneNode::Instance()->addComponent(hStaticSN);
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				/*Mesh* mesh = pMeshInstance->m_hAsset.getObject<Mesh>();
				pMeshInstance->m_bbTransformed.setMinMax(mesh->m_minBB, mesh->m_maxBB, pSN->m_worldTransform);
				
				PhysicsComponent* pc = new PhysicsComponent(PhysicsComponent::BOX, pSN);
				PhysicsManager::getInstance().addComponent(pc);
				pc->box.max = pSN->m_base * mesh->m_maxBB;
				pc->box.min = pSN->m_base * mesh->m_minBB;
