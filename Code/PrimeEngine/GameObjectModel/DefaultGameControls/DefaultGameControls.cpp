...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	if (Event_LEFT_CLICK::GetClassId() == pEvt->getClassId()) {
		Handle h("EVENT", sizeof(Event_LEFT_CLICK));

		Event_LEFT_CLICK * clickEvt = new(h) Event_LEFT_CLICK;
		Event_LEFT_CLICK* pRealEvent = (Event_LEFT_CLICK*)(pEvt);
		clickEvt->m_x = pRealEvent->m_x;
		clickEvt->m_y = pRealEvent->m_y;

		m_pQueueManager->add(h, QT_GENERAL);

	}

	if (Event_KEY_T_HELD::GetClassId() == pEvt->getClassId())
	{

		Handle h("EVENT", sizeof(Event_DEBUG_TEXT));

		Event_DEBUG_TEXT* debugText = new(h) Event_DEBUG_TEXT;
