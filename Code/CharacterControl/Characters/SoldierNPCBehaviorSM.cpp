...
void SoldierNPCBehaviorSM::do_SoldierNPCMovementSM_Event_TARGET_REACHED(PE::Events::Event *pEvt)
{
	...
	if (m_state == PATROLLING_WAYPOINTS)
	{
		...
		if (pGameObjectManagerAddon)
		{
			...
			std::string next[3] = {pWP->m_nextWayPointName, pWP->m_nextWayPointName2, pWP->m_nextWayPointName3 };
			std::string nextWP;
			...
			if (pWP && StringOps::length(pWP->m_nextWayPointName) > 0)
			{
				/*int rInd = rand() % 3;
				nextWP = next[rInd];*/
				if (StringOps::length(pWP->m_nextWayPointName2) == 0 && StringOps::length(pWP->m_nextWayPointName3) == 0) {
					nextWP = next[0];
				}
				else {
					int rInd = rand() % 3;
				...
				pWP = pGameObjectManagerAddon->getWayPoint(nextWP.c_str());
				if (pWP)
				{
...
void SoldierNPCBehaviorSM::do_PRE_RENDER_needsRC(PE::Events::Event *pEvt)
{
	...
	/*char buf[80];
	sprintf(buf, "%s Animation", m_isPartialAnim? "Partial" : "Full Body");
	SoldierNPC* pSol = getFirstParentByTypePtr<SoldierNPC>();
	...
	if (m_havePatrolWayPoint)
	{
		...
		/*PhysicsComponent* pc = hSoldierSceneNode.getObject<SceneNode>()->pc;
		if (pc) {
			pc->setCenter(base.getPos()); // Update physics position
		}

		Vector3 center = pc->sphere.center;
		float i = 1;

		Vector3 lineData[] = {
			center, Vector3(1, 0, 0), center + Vector3(i, 0, 0), Vector3(1, 0, 0), // X-axis (Red)
			center, Vector3(0, 1, 0), center + Vector3(0, i, 0), Vector3(0, 1, 0), // Y-axis (Green)
			center, Vector3(0, 0, 1), center + Vector3(0, 0, i), Vector3(0, 0, 1)  // Z-axis (Blue)
		};
		DebugRenderer::Instance()->createLineMesh(true, base, &lineData[0].m_x, 6, 0);
		*/
