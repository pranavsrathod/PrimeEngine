...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	/*
	PhysicsComponent* pc = new PhysicsComponent(PhysicsComponent::SPHERE, pMainSN);
	pc->setCenter(pEvt->m_pos); // Set collider position
	pc->sphere.radius = 1; // Set physics sphere size
	PhysicsManager::getInstance().addComponent(pc); // Register in PhysicsManager
	pMainSN->pc = pc; // Attach to SceneNode
	/*pMainSN->addComponent(pc);
	*/



	
	//PE::Handle hPhysics("PhysicsComponent", sizeof(PhysicsComponent));
	//PhysicsComponent* pPhysics = new(hPhysics) PhysicsComponent(
	//	*m_pContext, m_arena, hPhysics, PhysicsComponent::SPHERE, pMainSN);

	//pPhysics->setCenter(pEvt->m_pos); // Set physics collider position
	//PhysicsManager::getInstance().addComponent(pPhysics); // Register in PhysicsManager
	//pMainSN->addComponent(hPhysics); // Attach PhysicsComponent to SceneNode
	...
	pSoldierBehaviorSM->m_isPartialAnim = pEvt->m_isTarget;
	/*PEINFO("is Partial Animation?: %b", pSoldierBehaviorSM->m_isPartialAnim);*/
